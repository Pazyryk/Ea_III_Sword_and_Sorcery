		...
		for i = 1, GameDefines.MAX_MAJOR_CIVS-1 do
			if( PreGame.GetSlotStatus( i ) ~= SlotStatus.SS_COMPUTER ) then
				...
				if( civIndex ~= -1 ) then
					...
					--Paz disabled: IconHookup( leader.PortraitIndex, 64, leader.IconAtlas, controlTable.Portrait );
					...
					--Paz disabled: IconHookup( 22, 64, "LEADER_ATLAS", controlTable.Portrait );
				...
				if civIndex ~= -1  then
					local civ = GameInfo.Civilizations[civIndex]
					raceKey = civ.Description
					IconHookup( civ.PortraitIndex, 64, civ.IconAtlas, controlTable.Icon )
				else
					raceKey = "TXT_KEY_EA_RANDOM_RACE"
					SimpleCivIconHookup(-1, 64, controlTable.Icon)
				end

				controlTable.CivNumberIndex:LocalizeAndSetText("TXT_KEY_NUMBERING_FORMAT", count + 1)
				controlTable.CivPulldown:GetButton():LocalizeAndSetText(raceKey)
...
function RefreshHumanPlayer()
    ...
    if( civIndex ~= -1 ) then
		...
		if(name ~= "") then
			...
			--Paz disabled: Controls.RemoveButton:SetHide(false);
		else
			Controls.CivName:SetHide(true);
			--Paz disabled: Controls.RemoveButton:SetHide(true);
		...
		--Paz disabled: IconHookup( leader.PortraitIndex, 64, leader.IconAtlas, Controls.Portrait );
		...
		--Paz disabled: IconHookup( 22, 64, "LEADER_ATLAS", Controls.Portrait );
	...
	Controls.CivName:SetHide(true)
	Controls.CivPulldown:SetHide(false)
	Controls.CivPulldown:GetButton():LocalizeAndSetText(raceKey)
	--end Paz modified
end
------------------------------------------------------------------------------------------------------
...
["Civs"] = {
	FullSync = function()
		...
		function GetPlayableCivInfo()
			...
			--[[Paz modified below (double brackets removed from sql)
		...
		function PopulateCivPulldown( playableCivs, pullDown, playerID )
			...
			--[[Paz modified below
			controlTable.Button:LocalizeAndSetText("TXT_KEY_EA_RANDOM_RACE");
			controlTable.Button:LocalizeAndSetToolTip("TXT_KEY_RANDOM_LEADER_HELP");
			]]
			controlTable.Button:LocalizeAndSetText("TXT_KEY_EA_RANDOM_RACE")
			controlTable.Button:LocalizeAndSetToolTip("TXT_KEY_EA_RANDOM_RACE_HELP")
			--end Paz modified
			...
			for id, civ in ipairs(playableCivs) do
				...
				controlTable.Button:LocalizeAndSetText(civ.Description)
				controlTable.Button:LocalizeAndSetToolTip(civ.EaRaceSelectionText)
				--end Paz modified
			end    
		...
		pullDown:RegisterSelectionCallback( function(id)
			...
			elseif(mapScript.IsEarthMap) then
				...
				if(mapScript.WBMapData ~= nil) then
					...
					--Paz disable: PreGame.SetEra(wb.StartEra);
...
["MinorCivs"] = {
	FullSync = function()
		function OnMinorCivsSlider( fValue )
			...
			local maxMinorCivs = math.min(24, #GameInfo.MinorCivilizations);	--Paz changed from 41
...
["Teams"] = {
	FullSync = function()
		...
		--Paz added: just copied PartialSync which updates the slot without pulldown
...
function OnStartClicked()
	--Paz disabled: PreGame.SetPersistSettings(true);
	--Paz add
	SaveEaSettings()
	SetEaRequiredOptions()
	SetEaCivs()
	--end Paz add
