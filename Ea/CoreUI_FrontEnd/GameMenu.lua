...
function PopulateGameData()
	...
	--Paz disabled: local leaderTrait = GameInfo.Leader_Traits( "LeaderType ='" .. leader.Type .. "'" )();
	--Paz disabled: local gameTrait = GameInfo.Traits[ leaderTrait.TraitType ];
	...
	if(Game:IsNetworkMultiPlayer() and pPlayer:GetNickName() ~= "") then
		...
		--Paz disabled: Controls.CivIconFrame:SetToolTipString( Locale.ConvertTextKey( myCivInfo.ShortDescription )  .. "     " .. Locale.ConvertTextKey( gameTrait.ShortDescription ) .. "[NEWLINE]" .. Locale.ConvertTextKey( gameTrait.Description ) );
	...
	elseif( iPlayer == Game.GetActivePlayer() and PreGame.GetCivilizationShortDescription( 0 ) ~= "" ) then
		--Paz disabled: Controls.CivIconFrame:SetToolTipString( PreGame.GetCivilizationShortDescription( 0 ) .. "     " .. Locale.ConvertTextKey( gameTrait.ShortDescription ) .. "[NEWLINE]" .. Locale.ConvertTextKey( gameTrait.Description ) );
	...
	for row in GameInfo.MapScripts{FileName = mapScript, Hidden = 0} do
		IconHookup( row.IconIndex or 0, 64, row.IconAtlas, Controls.TypeIcon );
	...
	if(not bFound) then
		...
		for i,v in ipairs(earthMaps) do
			if(v == mapScript) then
				IconHookup(3, 64, "WORLDTYPE_ATLAS", Controls.TypeIcon);
