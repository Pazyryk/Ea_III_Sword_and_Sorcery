...
-------------------------------------------------
include( "IconSupport" );
--Paz disable: include( "UniqueBonuses" );

--Paz add
include("EaSetupFunctions.lua")
local g_bHasLoadedEaSettings = false
--end Paz add
...
-------------------------------------------------
function OnBack()
	--Paz add: persist settings to EaSetupData
	SaveEaSettings()
	--end Paz add
...
function OnSpeed()
	...
	Controls.SelectCivilization:SetHide(true)	--Paz added
...
function UpdateDisplay()
	...
	--[[Paz disabled (handled by LoadEaSettings)
...
function SetSelectedCiv()
    ...
    if( civ == nil ) then
		...
		raceHelpKey = "TXT_KEY_EA_RANDOM_RACE_HELP"
		mapTexture = "EaGameSetupImage.dds"		--"MapRandom512.dds"	
	else
		  
		IconHookup( civ.PortraitIndex, 128, civ.IconAtlas, Controls.Portrait )

		--?? SimpleCivIconHookup( 0, 64, Controls.IconShadow );      
         
		raceKey = civ.Description
		raceHelpKey = civ.EaRaceSelectionText	
        mapTexture = civ.MapImage 

    end
    -- Set Title string
    Controls.Title:SetText(Locale.ConvertTextKey("TXT_KEY_EA_RACE_SETTING", Locale.ConvertTextKey(raceKey)))

	-- Set Race help string
	Controls.BonusDescription:SetText( Locale.ConvertTextKey( raceHelpKey ))
		   
    -- Set Selected Civ Map (will be race image)
	Controls.LargeMapImage:UnloadTexture()
	Controls.LargeMapImage:SetTexture(mapTexture)
...
function ShowHideHandler( isHide )
    --if( isHide == false ) then 	
		...
		--Paz add
		if not g_bHasLoadedEaSettings then
			LoadEaSettings()				--Only run once
			g_bHasLoadedEaSettings = true
		end
		--end Paz add
		...
		if(bIsModding) then
			--Paz modded to line below: Controls.ScreenTitle:SetText( Locale.ConvertTextKey( "TXT_KEY_MODDING_SETUP_TITLE" ) );
			Controls.ScreenTitle:SetText( Locale.ConvertTextKey( "TXT_KEY_EA_MODDING_SETUP_TITLE" ) );
		else
