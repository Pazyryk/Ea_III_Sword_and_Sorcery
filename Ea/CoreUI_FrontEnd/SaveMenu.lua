...
function ShowHideHandler( isHide )
    if( not isHide ) then
        ...
        --Paz modified below: Controls.NameBox:SetText(leaderDescription.."_"..Game.GetTimeString());
		local eaPlayer = gT.gPlayers[iPlayer]
		local strEaCivRace
		local raceInfo = GameInfo.EaRaces[eaPlayer.race]
		local nameTrait = eaPlayer.eaCivNameID
		if nameTrait then
			strEaCivRace = Locale.Lookup(PreGame.GetCivilizationShortDescription(iPlayer)) .." ("..Locale.ConvertTextKey(raceInfo.Description)..")"
		else
			strEaCivRace = Locale.Lookup(PreGame.GetCivilizationShortDescription(iPlayer))
		end
		local strEaLeader = Locale.Lookup(PreGame.GetLeaderName(iPlayer))

		local saveStr = strEaCivRace..", "..strEaLeader..", Year "..Game.GetGameTurn()
		Controls.NameBox:SetText(saveStr)
		--end Paz modified
