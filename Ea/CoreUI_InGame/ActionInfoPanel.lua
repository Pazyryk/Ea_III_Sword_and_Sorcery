...
function OnEndTurnClicked()
	...
	print("OnEndTurnClicked iPlayer=", iPlayer)
	...
	--Paz add: mod turn blockers  (how do we set blocking text???)
	elseif eaPlayer.turnBlockEaCivNamingID then
		LuaEvents.EaAchievedCivNamePopup(iPlayer, eaPlayer.turnBlockEaCivNamingID)		
	--end Paz add
		...
		--Paz disabled: local iEndTurnControl = GameInfoTypes.CONTROL_ENDTURN;
		--Paz disabled: Game.DoControl(iEndTurnControl)
		
		--Paz add
		LuaEvents.EaPeoplePeopleAfterTurn(Game.GetActivePlayer(), true)	--this function will end turn when GPs are ready
		--end Paz modified		
			
	end
end
Controls.EndTurnButton:RegisterCallback( Mouse.eLClick, OnEndTurnClicked );
--Paz add: "return" button from main context routed through here, rather than (?) dll
LuaEvents.ActionInfoPanelOnEndTurnClicked.Add( OnEndTurnClicked )
--end Paz add
