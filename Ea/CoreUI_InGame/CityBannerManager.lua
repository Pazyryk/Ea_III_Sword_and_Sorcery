...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	if city ~= nil then
		...
		local convertedKey = localizedCityName	--Paz modified from:  = Locale.ToUpper(localizedCityName);
		...
		if (eReligion >= 0) then
			...
			if city:IsHolyCityForReligion(eReligion) then
				...
				iconAtlas = "EA_RELIGION_ATLAS"
			end
			IconHookup( religion.PortraitIndex, 32, iconAtlas, controls.ReligiousIcon )
			IconHookup( religion.PortraitIndex, 32, iconAtlas, controls.ReligiousIconShadow )
			--end Paz modified		
		end	
		...
		if garrisonedUnit == nil then
			if isActiveTeamCity then
				--Paz add
				--if not controls.GarrisonFrame then return end	--this can happen when shifting to autoplay
				--end Paz add
...
function OnCityCreated( hexPos, playerID, cityID, cultureType, eraType, continent, populationSize, size, fowState )
	...
	if(iActiveTeam ~= team and Game.GetAIAutoPlay() == 0) then		--Paz added autoplay test
