...
function GetCityStateStatusToolTip(iMajor, iMinor, bFullInfo)
	--print("PazDebug CityStateStatusHelper GetCityStateStatusToolTip ", iMajor, iMinor, bFullInfo)
	local pMajor = Players[iMajor];
	local pMinor = Players[iMinor];
	...
	--Paz disabled: local iInfluenceChangeThisTurn = pMinor:GetFriendshipChangePerTurnTimes100(iMajor) / 100;
	--Paz disabled: local iInfluenceAnchor = pMinor:GetMinorCivFriendshipAnchorWithMajor(iMajor);
	--Paz add
	local iInfluenceChangeThisTurn = 0
	local iInfluenceAnchor = 0
	if gT.gPlayers then		--mod inited
		local bGod = MapModData.playerType[iMinor] == "God"
		if bGod then
			iInfluenceAnchor = 0
		else
			local eaMajor = gT.gPlayers[iMajor]
			if not eaMajor then return "" end
			local eaMinor = gT.gPlayers[iMinor]
			iInfluenceAnchor = csBaselineRelationshipByRace[eaMajor.race][eaMinor.race]
		end
		if iInfluence < iInfluenceAnchor then
			iInfluenceChangeThisTurn = 1	
		elseif iInfluence > iInfluenceAnchor then
			iInfluenceChangeThisTurn = pMinor:GetPersonality() == MinorCivPersonalityTypes.MINOR_CIV_PERSONALITY_HOSTILE and -1.5 or -1
			if not bGod and pMajor:HasPolicy(GameInfoTypes.POLICY_PATRONAGE) then
				iInfluenceChangeThisTurn = iInfluenceChangeThisTurn / 2
			end
		end
	end
	--end Paz add
