...
--Paz add
local g_bConscriptOrIndentureDisabled = false
--end Paz add
...
function AddBuildingButton( pCity, building )
	--print("PazDebug CityView AddBuildingButton")
	local buildingID= building.ID;
	if (pCity:IsHasBuilding(buildingID)) then

		--Paz add: don't show hidden (text will allow some to be revealed under specific circumstances)
		if building.EaHidden and not MapModData.bDebugShowHiddenBuildings then
			return
		end
		--end Paz add	
		...
		if building.SpecialistType then
			...
			--[[Paz modified below to use Ea specialists
			...
			if building.SpecialistType == "SPECIALIST_SCRIBE" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(scientistTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(scientistTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(scientistTexture);
			elseif building.SpecialistType == "SPECIALIST_TRADER" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(merchantTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(merchantTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(merchantTexture);
			elseif building.SpecialistType == "SPECIALIST_ARTISAN" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(artistTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(artistTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(artistTexture);
			elseif building.SpecialistType == "SPECIALIST_SMITH" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(engineerTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(engineerTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(engineerTexture);
			elseif building.SpecialistType == "SPECIALIST_DISCIPLE" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(artistTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(artistTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(artistTexture);
			else
...
function OnCityViewUpdate()
	--print("PazDebug CityView OnCityViewUpdate")
	...
	if (pCity ~= nil) then
		...
		--Paz add
		local eaPlayer = gT.gPlayers[pPlayer:GetID()]
		...
		if (bEnableConscript or bEnableIndenture) and not pCity:IsRazing() then
			...
			--add dynamic placement to account for raze button
		else
			g_bConscriptOrIndentureDisabled = true
			Controls.ConscriptOrIndentureButton:SetHide(true)
		end
...
--		UpdateSpecialists(pCity);
		...
		if eaPlayer.bUsesDivineFavor then
			Controls.ManaBox:SetHide(true)
			Controls.FaithBox:SetHide(false)
			local iFaithPerTurn = pCity:GetFaithPerTurn() + pCity:GetBaseYieldRateFromMisc(YieldTypes.YIELD_FAITH)
			Controls.FaithPerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT", iFaithPerTurn) )
		else
		...
		if (UI.IsCityScreenViewingMode()) then
			...
			--Paz add
			Controls.ConscriptOrIndentureButton:SetDisabled(true)
			--end Paz add
			...
			--Paz add
			if not g_bConscriptOrIndentureDisabled then
				Controls.ConscriptOrIndentureButton:SetDisabled(false)
			end
			--end Paz add
...
-------------------------------------------------
-------------------------------------------------
function OnConscriptOrIndentureButton()
	local city = UI.GetHeadSelectedCity()
	if not city then return end
	local popupInfo = {
		Type = ButtonPopupTypes.BUTTONPOPUP_MODDER_1,
		Data1 = -1,
		Data2 = -1,
		Data3 = -1,
		Option1 = false,
	...
	Events.SerialEventGameMessagePopup(popupInfo)	--EaGenericPopupConscript.lua	
end
Controls.ConscriptOrIndentureButton:RegisterCallback( Mouse.eLClick, OnConscriptOrIndentureButton)
