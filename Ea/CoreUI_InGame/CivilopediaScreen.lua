...
CivilopediaCategory[CategoryUnits].PopulateList = function()
	...
	-- for each era
	local sql = --Paz: removed enclosing bracket quotes
	...
	--first non-race, then sort by race
	sortedList[CategoryUnits][0] = {}
	local tableid = 1
	for unit in DB.Query("SELECT Units.ID, Units.Description, Units.PortraitIndex, Units.IconAtlas FROM Units WHERE EaRace IS NULL AND Special IS NULL") do
		AddArticle(0, tableid, unit)
		tableid = tableid + 1
	end

	local sectionID = 1
	for race in GameInfo.EaRaces() do
		if race.PediaCategoryText then		--these are civ races
			sortedList[CategoryUnits][sectionID] = {}
			local tableid = 1
			local sql = [[SELECT Units.ID, Units.Description, Units.PortraitIndex, Units.IconAtlas FROM Units WHERE EaRace = ']]..race.Type..[[';]]
			for unit in DB.Query(sql) do
				AddArticle(sectionID, tableid, unit)
				tableid = tableid + 1
			end
			sectionID = sectionID + 1
		end
...
CivilopediaCategory[CategoryCivilizations].PopulateList = function()
	...
	for row in GameInfo.Civilizations() do
		if row.Type ~= "CIVILIZATION_MINOR" and row.Type ~= "CIVILIZATION_BARBARIAN" and row.Type ~= "CIVILIZATION_ANIMALS" then	--Paz added Animals
			local article = {};
...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		local classOverrideCondition = string.format("UnitType='%s' and CivilizationType <> 'CIVILIZATION_BARBARIAN' and CivilizationType <> 'CIVILIZATION_MINOR' and CivilizationType <> 'CIVILIZATION_ANIMALS'", thisUnit.Type);	--Paz added Animals
...
function SelectBuildingOrWonderArticle( buildingID )
	if buildingID ~= -1 then
		...
		for row in GameInfo.Civilization_BuildingClassOverrides( condition ) do
			if row.CivilizationType ~= "CIVILIZATION_BARBARIAN" and row.CivilizationType ~= "CIVILIZATION_MINOR" and row.Type ~= "CIVILIZATION_ANIMALS" then	--Paz added Animals
...
CivilopediaCategory[CategoryCivilizations].SelectArticle = function( rawCivID, shouldAddToList )
	...
	if rawCivID < 1000 then
		if rawCivID ~= -1 then
			...
			if thisCiv and thisCiv.Type ~= "CIVILIZATION_MINOR" and thisCiv.Type ~= "CIVILIZATION_BARBARIAN" and row.Type ~= "CIVILIZATION_ANIMALS" then	--Paz added Animals
...
CivilopediaCategory[CategoryUnits].SelectHeading = function( selectedEraID, dummy )
	...
	PopulateHeaderAndItems(0, "TXT_KEY_EA_ALL_RACES")
	local sectionID = 1
	for race in GameInfo.EaRaces() do
		if race.PediaCategoryText then
			PopulateHeaderAndItems(sectionID, race.PediaCategoryText)
			sectionID = sectionID + 1
		end
	end
	--end Paz modified
...
CivilopediaCategory[CategoryUnits].DisplayList = function()
	...
	PopulateAndAdd(0, "TXT_KEY_EA_ALL_RACES")
	local sectionID = 1
	for race in GameInfo.EaRaces() do
		if race.PediaCategoryText then
			PopulateAndAdd(sectionID, race.PediaCategoryText)
			sectionID = sectionID + 1
		end
	end
	--end Paz modified
