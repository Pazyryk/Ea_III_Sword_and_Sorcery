...
function OnCultureOverview()
    local popupInfo = {
        --Paz modified below: Type = ButtonPopupTypes.BUTTONPOPUP_CULTURE_OVERVIEW,
...
function OnEspionageButton()
	...
	LuaEvents.ShowEaCivilizationsPopup()
	--end Paz modified
end
...
--Paz add: Use the espionage button to activate civ screen (hide with civ naming)
function CheckEaCivilizationsButton()
	local eaPlayer = gT.gPlayers and gT.gPlayers[Game.GetActivePlayer()]
	local bShow = not (eaPlayer and eaPlayer.eaCivNameID)			--Needed before a civ has a name
	Controls.CornerAnchor:SetHide(bShow)
	Controls.CornerAnchor_Espionage:SetHide(not bShow)
	Controls.EspionageButton:SetHide(not bShow)
end
Events.ActivePlayerTurnStart.Add(CheckEaCivilizationsButton)

CheckEaCivilizationsButton()
