...
function OnTechPanelUpdated()
	--print("PazDebug OnTechPanelUpdated")
	local pPlayer = Players[Game.GetActivePlayer()];
	local pTeam = Teams[pPlayer:GetTeam()];
	local pTeamTechs = pTeam:GetTeamTechs();
	
	eCurrentTech = pPlayer:GetCurrentResearch();
	--Paz disabled: eRecentTech = pTeamTechs:GetLastTechAcquired();
	--Paz add
	local recentTechAny = pTeamTechs:GetLastTechAcquired()
	if recentTechAny ~= -1 then
		local recentTechAnyInfo = GameInfo.Technologies[recentTechAny]
		if not recentTechAnyInfo.Utility then
			eRecentTech = recentTechAny
		end
	end
	--end Paz add
