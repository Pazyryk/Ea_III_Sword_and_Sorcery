...
-- Tech Tree Popup
-------------------------------------------------
--Paz note: This is modified from G&K file (never updated to BNW)
...
local g_tierOffset = 0	--used for left/right toggle
local bInited = false
--end Paz add

local maxTechNameLength = 27 - Locale.Length(Locale.Lookup("TXT_KEY_TURNS"));	--Paz: changed from 22 -
...
function InitialSetup()
	...
	--Paz disabled: Controls.TechTreeScrollBar:SetSizeX( Controls.TechTreeScrollPanel:GetSize().x - 150 );
	...
	for row in GameInfo.Technology_PrereqTechs() do
		...
		if tech and prereq then
			...
			if tech.GridX - prereq.GridX > 1 then
				...
				hConnection.TechPipeIcon:SetOffsetVal(prereq.GridX*blockSpacingX + blockSizeX + 96 + g_tierOffset, (prereq.GridY-5)*blockSpacingY + 12 + extraYOffset);	--Paz added g_tierOffset
			...
			if tech.GridY - prereq.GridY == 1 or tech.GridY - prereq.GridY == -1 then
				...
				vConnection.TechPipeIcon:SetOffsetVal((tech.GridX-1)*blockSpacingX + blockSizeX + 96 + g_tierOffset, ((tech.GridY-5)*blockSpacingY) - (((tech.GridY-prereq.GridY) * blockSpacingY) / 2) + extraYOffset);	--Paz added g_tierOffset
	...
	for pipeIndex, thisPipe in pairs(techPipes) do
		...
		if thisPipe.rightConnectionType >= 1 then
			...
			startPipe.TechPipeIcon:SetOffsetVal( tech.GridX*blockSpacingX + blockSizeX + 64 + g_tierOffset, yOffset );	--Paz added g_tierOffset
			...
			if thisPipe.rightConnectionType == 1 then
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX)*blockSpacingX + blockSizeX + 96 + g_tierOffset, yOffset );	--Paz added g_tierOffset
				pipe.TechPipeIcon:SetTexture(right1Texture);
			elseif thisPipe.rightConnectionType == 2 then
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX+thisPipe.xOffset)*blockSpacingX + blockSizeX + 96 - 12 + g_tierOffset, yOffset - 15 );	--Paz added g_tierOffset
			...
			elseif thisPipe.rightConnectionType == 4 then
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX+thisPipe.xOffset)*blockSpacingX + blockSizeX + 96 - 12 + g_tierOffset, yOffset );	--Paz added g_tierOffset
			...
			elseif thisPipe.rightConnectionType == 5 then
				...
				pipe.TechPipeIcon:SetTexture(right1Texture);
				pipe = g_PipeManager:GetInstance();			
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX)*blockSpacingX + blockSizeX + 96 - 12 + g_tierOffset, yOffset );	--Paz added g_tierOffset
				pipe.TechPipeIcon:SetTexture(topRightTexture);
				pipe.TechPipeIcon:SetTextureOffset(Vector2(72,0));
		...
		if thisPipe.leftConnectionType >= 1 then
			...
			local startPipe = g_PipeManager:GetInstance();
			startPipe.TechPipeIcon:SetOffsetVal( tech.GridX*blockSpacingX + 26 + g_tierOffset, yOffset );	--Paz added g_tierOffset
			startPipe.TechPipeIcon:SetTexture(left1Texture);
			startPipe.TechPipeIcon:SetSize(	Vector2(40, 42) );
			
			local pipe = g_PipeManager:GetInstance();			
			if thisPipe.leftConnectionType == 1 then
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX)*blockSpacingX + g_tierOffset, yOffset );	--Paz added g_tierOffset
				pipe.TechPipeIcon:SetTexture(left1Texture);
			elseif thisPipe.leftConnectionType == 2 then
				pipe.TechPipeIcon:SetOffsetVal( (tech.GridX)*blockSpacingX + g_tierOffset, yOffset - 15);	--Paz added g_tierOffset
	...
	--Paz disable: Controls.EraStack:CalculateSize();
	--Paz disable: Controls.EraStack:ReprocessAnchoring();
    --Paz disable: Controls.TechTreeScrollPanel:CalculateInternalSize();
    

	--Paz add
	if not bInited then
		local scrollPanelSize = Controls.TechTreeScrollPanel:GetSize()
		--scrollPanelSize.x = screenSizeX - 100
		scrollPanelSize.y = screenSizeY - 70
		Controls.TechTreeScrollPanel:SetSize( scrollPanelSize )
		Controls.TechTreeScrollBar:SetSizeY( screenSizeY - 106 )
		Controls.TechTreeScrollPanel:CalculateInternalSize()
		bInited = true
		extraYOffset = -450		--don't know why this is needed, but it is
	end
...
function AddTechButton( tech )
	...
	if thisTechButtonInstance then
		...
		thisTechButtonInstance.TechButton:SetOffsetVal( tech.GridX*blockSpacingX + 64 + g_tierOffset, (tech.GridY-5)*blockSpacingY + extraYOffset);	--Paz added g_tierOffset
...
--Paz add: Toggle buttons
--------------------------------------------------------------------------------
-- ToggleLeft clicked event handler
--------------------------------------------------------------------------------
function OnToggleLeftClicked()
	if g_tierOffset / blockSpacingX < 0 then
		g_tierOffset = g_tierOffset + blockSpacingX
		for i, v in pairs(techButtons) do
			techButtons[i] = nil
		end
...
Controls.ToggleLeft:RegisterCallback( Mouse.eLClick, OnToggleLeftClicked )


--------------------------------------------------------------------------------
-- ToggleRight clicked event handler
--------------------------------------------------------------------------------
function OnToggleRightClicked()
	if g_tierOffset / blockSpacingX > -5 then
		g_tierOffset = g_tierOffset - blockSpacingX
		for i, v in pairs(techButtons) do
			techButtons[i] = nil
		end
...
Controls.ToggleRight:RegisterCallback( Mouse.eLClick, OnToggleRightClicked )
