...
--Paz global replacements (did all case combinations)
--
--copper -> jade
--coal -> copper
--aluminum -> mithril
...
--oil -> naphtha
--truffles -> boars
--ivory -> elephant (it's really a strategic now, but it will indirectly give ivory and I like it distributed clumpy like a luxury)
		

--uran_amt -> cinnabar_amt
--alum_amt -> mithril_amt

--Added below following existing templates:

--luxury
--tobacco (grass,plains)
--tea (grass, hills)
--poppies (plains, desert, flat)

--bonus
--berries (grass, tundra, flat, hills)


--Marble impact system converted to Moly (Marble treated as regular luxury)
--Changed boars to bonus (was luxury after replacing truffles)
--Changed crabs to bonus

--Changed wool to luxury
--added back Marble as a normal Luxury

--All other changes are marked below

include("MapmakerUtilities");
include("NaturalWondersCustomMethods");
--Paz add
include("EaStartingPlotsFunctions.lua")
--end Paz add
...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		PlaceMagics = AssignStartingPlots.PlaceMagics,
		...
		--Paz disable:	PlaceNaphthaInTheSea = AssignStartingPlots.PlaceNaphthaInTheSea,
		...
		--Paz add
		RemoveBerriesInDesert = AssignStartingPlots.RemoveBerriesInDesert,
		--end Paz add
		...
		amounts_of_resources_placed = table.fill(0, 50), -- Stores amounts of each resource ID placed. WARNING: This table uses adjusted resource ID (+1) to account for Lua indexing. Add 1 to all IDs to index this table.
		luxury_assignment_count = table.fill(0, 50), -- Stores amount of each luxury type assigned to regions. WARNING: current implementation will crash if a Luxury is attached to resource ID 0 (default = iron), because this table uses unadjusted resource ID as table index.
		luxury_low_fert_compensation = table.fill(0, 50), -- Stores number of times each resource ID had extras handed out at civ starts. WARNING: Indexed by resource ID.
		...
		magicsData = table.fill(0, iW * iH), -- Stores "impact and ripple" data in the magics layer
		--Paz no used: sheepData = table.fill(0, iW * iH), -- Stores "impact and ripple" data in the sheep layer -- Sheep use regular bonus layer PLUS this one
		...
		iNumTypesUnassigned = 22,		-- Total number of luxuries. Adjust if modifying number of luxury resources.	--Paz changed from 20
		...
		iNumTypesSpecialCase = 1,		-- Magics affects Wonder construction, so requires special-case handling
		...
		iron_ID, horse_ID, copper_ID, naphtha_ID, mithril_ID, cinnabar_ID,
		whale_ID, pearls_ID, elephants_ID, fur_ID, silk_ID,
		dye_ID, spices_ID, sugar_ID, cotton_ID, wine_ID, incense_ID,
		gold_ID, silver_ID, gems_ID, marble_ID,
		--Paz added:
		yew_ID, moly_ID, tobacco_ID, tea_ID, opium_ID, berries_ID,
		-- Expansion luxuries
		jade_ID, salt_ID, citrus_ID, boars_ID, crab_ID,
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		elseif resourceType == "RESOURCE_WOOL" then		--Paz changed to luxury distribution
		...
		elseif resourceType == "RESOURCE_NAPHTHA" then
			self.naphtha_ID = resourceID;
		elseif resourceType == "RESOURCE_MITHRIL" then
			self.mithril_ID = resourceID;
		elseif resourceType == "RESOURCE_QUICKSILVER" then
			self.cinnabar_ID = resourceID;
		elseif resourceType == "RESOURCE_YEW" then
			self.yew_ID = resourceID;
		-- Set up Luxury IDs
		...
		elseif resourceType == "RESOURCE_ELEPHANT" then
			self.elephants_ID = resourceID;
		...
		elseif resourceType == "RESOURCE_MARBLE" then	--Paz changed: impact system given to Magics; Marble edited to be regular luxury
		...
		elseif resourceType == "RESOURCE_BOARS" then	--Paz changed to bonus distribution	
			self.boars_ID = resourceID;
		elseif resourceType == "RESOURCE_CRAB" then		--Paz changed to bonus distribution
			self.crab_ID = resourceID;
		-- Paz additions
		elseif resourceType == "RESOURCE_TOBACCO" then	--luxury
			self.tobacco_ID = resourceID;
		elseif resourceType == "RESOURCE_TEA" then		--luxury
			self.tea_ID = resourceID;
		elseif resourceType == "RESOURCE_OPIUM" then	--luxury
			self.opium_ID = resourceID;
		elseif resourceType == "RESOURCE_BERRIES" then	--bonus
			self.berries_ID = resourceID;
		elseif resourceType == "RESOURCE_MOLY" then		--strategic with impact system taken from Marble
			self.moly_ID = resourceID;
		end
	end
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:GenerateRegions(args)
	...
	--Paz add: reserve last civ for The Fay (a hidden civ); override city state number
	self.iNumCivs = self.iNumCivs - 1
	local EaSetupDB = Modding.OpenUserData("EaSetupData", 1)
	self.iNumCityStates = EaSetupDB.GetValue("NUMBER_MINOR_CIVS")
	local maxAvailableCityStates = GetMaxAvailableCityStates()
	if self.iNumCityStates > maxAvailableCityStates then
		print("Ran out of city states so using max number available")
		self.iNumCityStates = maxAvailableCityStates
	end
	print("Number city states = ", self.iNumCityStates)
	--end Paz add
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	self:PlaceResourceImpact(x, y, 6, 0) -- Natural Wonders layer, set a minimum distance of 5 plots (4 ripples) away. Paz: changed 4 to 1 to allow Natural Wonders neer starts.
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis)
	...
	local plotType = plot:GetPlotType()
	local iW, iH = Map.GetGridSize();
	local latitude = math.abs((y - (iH / 2)) / (iH / 2))
	--
	if featureType == FeatureTypes.FEATURE_JUNGLE then -- Place Banana or Boars
		--Paz added boars on jungle hills
		if plotType == PlotTypes.PLOT_HILLS then
			plot:SetResourceType(self.boars_ID, 1);
			--print("Placed Boars.");
			self.amounts_of_resources_placed[self.boars_ID + 1] = self.amounts_of_resources_placed[self.boars_ID + 1] + 1;
			return true, false
		else
	...
	elseif featureType == FeatureTypes.FEATURE_FOREST then -- Place Deer or Boars
		if latitude < 0.35 then 
			plot:SetResourceType(self.boars_ID, 1);
			--print("Placed Boars.");
			self.amounts_of_resources_placed[self.boars_ID + 1] = self.amounts_of_resources_placed[self.boars_ID + 1] + 1;
			return true, false
	...
	elseif plotType == PlotTypes.PLOT_HILLS and featureType == FeatureTypes.NO_FEATURE then
		...
		if terrainType ~= TerrainTypes.TERRAIN_DESERT then
			plot:SetResourceType(self.berries_ID, 1);
			--print("Placed Berries.");
			self.amounts_of_resources_placed[self.berries_ID + 1] = self.amounts_of_resources_placed[self.berries_ID + 1] + 1;
			return true, false
		end
	...
	elseif plotType == PlotTypes.PLOT_OCEAN then
		if terrainType == TerrainTypes.TERRAIN_COAST and featureType == FeatureTypes.NO_FEATURE then
			if plot:IsLake() == false then -- Place Fish
				--Paz added Crab for extreem latitudes

				if latitude > 0.65 then
					plot:SetResourceType(self.crab_ID, 1);
					--print("Placed Crab.");
					self.amounts_of_resources_placed[self.crab_ID + 1] = self.amounts_of_resources_placed[self.crab_ID + 1] + 1;
					return true, false
				else
...
function AssignStartingPlots:AttemptToPlaceSmallStrategicAtPlot(x, y)
	...
	elseif featureType == FeatureTypes.NO_FEATURE then
		if terrainType == TerrainTypes.TERRAIN_GRASS or terrainType == TerrainTypes.TERRAIN_PLAINS then -- Could be horses.
			...
			if diceroll == 2 then
				choice = self.copper_ID;	--Paz: changed from iron
				--print("Placed Iron.");
			else
				--print("Placed Horse.");
			end
			...
			plot:SetResourceType(self.copper_ID, 2);	--Paz: changed from iron
			self.amounts_of_resources_placed[self.copper_ID + 1] = self.amounts_of_resources_placed[self.copper_ID + 1] + 2;	--Paz: changed from iron
			--print("Placed Iron.");
		end
		return true
	end
...
------------------------------------------------------------------------------
function AssignStartingPlots:AddStrategicBalanceResources(region_number)	--Paz note: no adjustments for strategic balance yet
	...
	local even = self.firstRingYIsEven;
	local nextX, nextY, plot_adjustments;
	local iron_list, horse_list, naphtha_list = {}, {}, {};
	local iron_fallback, horse_fallback, naphtha_fallback = {}, {}, {};
	local radius = 3;
					...
					elseif plotType == PlotTypes.PLOT_LAND then
						if featureType == FeatureTypes.NO_FEATURE then
							if terrainType == TerrainTypes.TERRAIN_TUNDRA or terrainType == TerrainTypes.TERRAIN_DESERT then
								if ripple_radius < 3 then
									table.insert(naphtha_list, plotIndex)
								else
									table.insert(naphtha_fallback, plotIndex)
								end
								table.insert(iron_fallback, plotIndex)
								table.insert(horse_fallback, plotIndex)
	...
	local cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = self:GetMajorStrategicResourceQuantityValues()
	local shuf_list;
	local placed_iron, placed_horse, placed_naphtha = false, false, false;
	...
	if table.maxn(naphtha_list) > 0 then
		shuf_list = GetShuffledCopyOfTable(naphtha_list)
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.naphtha_ID, naphtha_amt, 1, 1, -1, 0, 0, shuf_list);
		if iNumLeftToPlace == 0 then
			placed_naphtha = true;
		end
	end
	...
	if placed_naphtha == false and table.maxn(naphtha_fallback) > 0 then
		shuf_list = GetShuffledCopyOfTable(naphtha_fallback)
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.naphtha_ID, naphtha_amt, 1, 1, -1, 0, 0, shuf_list);
	end
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:NormalizeStartLocation(region_number)
	...
	-- Add mandatory Iron, Horse, Naphtha to every start if Strategic Balance option is enabled.
	...
	--Paz add: quick fix
	iNumFoodBonusNeeded = iNumFoodBonusNeeded < 1 and 1 or iNumFoodBonusNeeded
	--end Paz add
...
function AssignStartingPlots:GenerateNaturalWondersCandidatePlotLists()
	...
	for row in GameInfo.Features() do
		if (row.NaturalWonder == true) and IsEaGodForNWInGame(row) then	--Paz added God test
...
function AssignStartingPlots:AttemptToPlaceNaturalWonder(wonder_number, row_number)
	...
	for loop, plotIndex in ipairs(candidate_plot_list) do
		if self.naturalWondersData[plotIndex] == 0 then -- No collision with civ start or other NW, so place wonder here!
			...
			self:PlaceResourceImpact(x, y, 7, 1)					-- Magics layer
			local plotIndex = y * iW + x + 1;
...
function AssignStartingPlots:PlaceNaturalWonders()
	...
	local worldsizes = {
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = 4,	--Paz doubled
...
function AssignStartingPlots:AssignCityStatesToRegionsOrToUninhabited(args)
	...
	if self.method == 3 then -- Rectangular regional division spanning the entire globe, ALL plots belong to inhabited regions.
		...
		for x = 0, iW - 1 do
			for y = 3, iH - 1 do	--Paz: first 3 rows researved for the gods
				local plotIndex = y * iW + x + 1;
				local plot = Map.GetPlot(x, y);
				local plotType = plot:GetPlotType()
				local terrainType = plot:GetTerrainType()
			...
			for areaID, area_plot_list in pairs(land_area_plot_tables) do
				if TestMembership(areas_uninhabited, areaID) == true then 
					for loop, plotIndex in ipairs(area_plot_list) do
						...
						local plot = Map.GetPlot(x, y);
						local terrainType = plot:GetTerrainType();
						if terrainType ~= TerrainTypes.TERRAIN_SNOW and y > 2 then	--Paz: first 3 rows researved for the gods
							if self.plotDataIsCoastal[plotIndex] == true then
...
function AssignStartingPlots:PlaceCityStateInRegion(city_state_number, region_number)
	...
	if placed_city_state == true then
		...
		local city_state_ID = GetNextCityStateID()		--Paz changed from: city_state_number + GameDefines.MAX_MAJOR_CIVS - 1;
		...
		self:PlaceResourceImpact(x, y, 3, 3) -- Bonus layer
		self:PlaceResourceImpact(x, y, 4, 3) -- Fish layer
		self:PlaceResourceImpact(x, y, 7, 3) -- Magics layer
		local impactPlotIndex = y * iW + x + 1;
		self.playerCollisionData[impactPlotIndex] = true;
		...
		if cs_number <= self.iNumCityStates then -- Make sure it's an active city state before processing.
			if region_number == -1 and iUninhabitedCandidatePlots > 0 then -- Assigned to areas outside of Regions.
				...
				if success == true then
					...
					self:PlaceResourceImpact(cs_x, cs_y, 7, 3) -- Magics layer
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
	...
	local temp_magics_list, temp_deer_list, temp_desert_wheat_list, temp_banana_list = {}, {}, {}, {};
	--
	for y = 3, iH - 1 do	--Paz: first 3 rows researved for the gods
		for x = 0, iW - 1 do
			...
			elseif plot:GetResourceType(-1) ~= -1 then
				...
				elseif plotType == PlotTypes.PLOT_HILLS and terrainType ~= TerrainTypes.TERRAIN_SNOW then
					...
					table.insert(temp_magics_list, i);
					if featureType == FeatureTypes.NO_FEATURE then
	...
	self.magics_list = GetShuffledCopyOfTable(temp_magics_list)
...
function AssignStartingPlots:PlaceResourceImpact(x, y, impact_table_number, radius)
	...
	-- Impact #s - 1 strategic - 2 luxury - 3 bonus - 4 fish - 5 city states - 6 natural wonders - 7 magics - 8 sheep
	local iW, iH = Map.GetGridSize();
	local wrapX = Map:IsWrapX();
	local wrapY = Map:IsWrapY();
	local impact_value = 99;
	...
	elseif impact_table_number == 7 then
		self.magicsData[impactPlotIndex] = 1;
	end
	if radius == 0 then
		return
	end
			...
			for direction_index = 1, 6 do
				for plot_to_handle = 1, ripple_radius do
					...
					elseif wrapY == false and (nextY < 0 or nextY >= iH) then -- Y is out of bounds.
						...
						elseif impact_table_number == 7 then
							self.magicsData[ringPlotIndex] = 1;
						end
					end
					currentX, currentY = nextX, nextY;
				end
...
function AssignStartingPlots:ProcessResourceList(frequency, impact_table_number, plot_list, resources_to_place)
	...
	for place_resource = 1, iNumResourcesToPlace do
		...
		--print("Paz debug ", use_this_res_index, iNumTotalPlots, res_ID[1], res_quantity[1], res_ID[2], res_quantity[2])
...
function AssignStartingPlots:AssignLuxuryRoles()
	...
	-- Assign Magics to special casing.
	table.insert(self.resourceIDs_assigned_to_special_case, self.moly_ID);
	table.insert(self.resourceIDs_assigned_to_special_case, self.cinnabar_ID);
	--Paz (special is no longer a luxury) self.iNumTypesUnassigned = self.iNumTypesUnassigned - 1;
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					elseif plotType == PlotTypes.PLOT_HILLS and terrainType ~= TerrainTypes.TERRAIN_SNOW then
						...
						if featureType == FeatureTypes.NO_FEATURE then
							...
							allowed_luxuries[self.sheep_ID] = true;
						end
					elseif plotType == PlotTypes.PLOT_LAND then
						if featureType == FeatureTypes.NO_FEATURE then
							if terrainType == TerrainTypes.TERRAIN_TUNDRA then
							...
							elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
								allowed_luxuries[self.marble_ID] = true;
								allowed_luxuries[self.elephants_ID] = true;
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.elephants_ID then
	...
	--elseif resource_ID == self.boars_ID then
	...
	elseif resource_ID == self.tobacco_ID then
		primary, secondary, tertiary, quaternary = 13, 12, 11, 9;
	elseif resource_ID == self.tea_ID then
		primary, secondary, tertiary, quaternary = 6, 12, 13, 7;
	elseif resource_ID == self.opium_ID then
		primary, secondary, tertiary = 11, 10, 12;
	elseif resource_ID == self.sheep_ID then
		primary, secondary, tertiary = 4, 11, 12;
	end
...
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceMagics()
	local magics_already_placed = self.amounts_of_resources_placed[self.moly_ID + 1] + self.amounts_of_resources_placed[self.cinnabar_ID + 1];
	local magics_target = math.ceil(self.iNumCivs * 3);	--Paz doubled amounts
	if self.resource_setting == 1 then
		magics_target = math.ceil(self.iNumCivs * 2);
	elseif self.resource_setting == 3 then
		magics_target = math.ceil(self.iNumCivs * 3.6);
	end
	local iNumMagicsToPlace = math.max(2, magics_target - magics_already_placed);
	local iW, iH = Map.GetGridSize();
	local iNumLeftToPlace = iNumMagicsToPlace;
	local iNumPlots = table.maxn(self.magics_list);
	if iNumPlots < 1 then
		--print("No eligible plots available to place Magics!");
		return
	end
	-- Main loop
	for place_resource = 1, iNumMagicsToPlace do
		for loop, plotIndex in ipairs(self.magics_list) do
			if self.magicsData[plotIndex] == 0 then --and self.luxuryData[plotIndex] == 0
				local x = (plotIndex - 1) % iW;
				local y = (plotIndex - x - 1) / iW;
				local res_plot = Map.GetPlot(x, y)
				local featureType = res_plot:GetFeatureType()
				if res_plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS and featureType ~= FeatureTypes.FEATURE_ICE then -- Placing this resource in this plot.
					local plotType = res_plot:GetPlotType()
					local terrainType = res_plot:GetTerrainType()
					if plotType == PlotTypes.PLOT_HILLS or terrainType == TerrainTypes.TERRAIN_SNOW then
						res_plot:SetResourceType(self.cinnabar_ID, 1);
						self.amounts_of_resources_placed[self.cinnabar_ID + 1] = self.amounts_of_resources_placed[self.cinnabar_ID + 1] + 1;
						print("-"); print("-Placed Cinnabar randomly at Plot", x, y);
					elseif terrainType == TerrainTypes.TERRAIN_DESERT and terrainType == TerrainTypes.TERRAIN_TUNDRA then
						--use flat desert and tundra to even out the two magic resources
						if self.amounts_of_resources_placed[self.moly_ID + 1] < self.amounts_of_resources_placed[self.cinnabar_ID + 1] then
							res_plot:SetResourceType(self.moly_ID, 1);
							self.amounts_of_resources_placed[self.moly_ID + 1] = self.amounts_of_resources_placed[self.moly_ID + 1] + 1;
							print("-"); print("-Placed Moly randomly at Plot", x, y);
						else
							res_plot:SetResourceType(self.cinnabar_ID, 1);
							self.amounts_of_resources_placed[self.cinnabar_ID + 1] = self.amounts_of_resources_placed[self.cinnabar_ID + 1] + 1;
							print("-"); print("-Placed Cinnabar randomly at Plot", x, y);
						end
					elseif terrainType == TerrainTypes.TERRAIN_PLAINS and not res_plot:IsFreshWater() then
						--as above but favor Moly here
						if self.amounts_of_resources_placed[self.moly_ID + 1] < 1.25 * self.amounts_of_resources_placed[self.cinnabar_ID + 1] then
							res_plot:SetResourceType(self.moly_ID, 1);
							self.amounts_of_resources_placed[self.moly_ID + 1] = self.amounts_of_resources_placed[self.moly_ID + 1] + 1;
							print("-"); print("-Placed Moly randomly at Plot", x, y);
						else
					...
					iNumLeftToPlace = iNumLeftToPlace - 1;
					self:PlaceResourceImpact(x, y, 7, 5)
					--res_plot:SetResourceType(self.moly_ID, 1);
					--self.amounts_of_resources_placed[self.moly_ID + 1] = self.amounts_of_resources_placed[self.moly_ID + 1] + 1;
					--print("-"); print("Placed Moly randomly at Plot", x, y);
					--self.totalLuxPlacedSoFar = self.totalLuxPlacedSoFar + 1;
					--iNumLeftToPlace = iNumLeftToPlace - 1;
					--print("Still need to place", iNumLeftToPlace, "more units of Moly.");
	...
	if iNumLeftToPlace > 0 then
		print("Failed to place", iNumLeftToPlace, "units of Magics.");
	end
end
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceSmallQuantitiesOfStrategics(frequency, plot_list)
	...
	local cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = self:GetSmallStrategicResourceQuantityValues()
					...
					if res_plot:GetResourceType(-1) == -1 then
						...
						if featureType == FeatureTypes.FEATURE_MARSH then
							...
							elseif diceroll == 1 then
								...
								selected_ID = self.naphtha_ID;
								selected_quantity = naphtha_amt;
							end
						elseif featureType == FeatureTypes.FEATURE_JUNGLE then
							local diceroll = Map.Rand(4, "Resource selection - Place Small Quantities LUA");
							if diceroll == 0 then
							...
							elseif diceroll == 1 then
								...
								selected_ID = self.mithril_ID;
								selected_quantity = mithril_amt;
							end
						elseif featureType == FeatureTypes.FEATURE_FOREST then
							local diceroll = Map.Rand(4, "Resource selection - Place Small Quantities LUA");	--Paz added yew (2 out of 4)
							if diceroll == 0 then
								selected_ID = self.copper_ID;
								selected_quantity = copper_amt;
							elseif diceroll == 1 then
								...
								selected_ID = self.yew_ID;
								selected_quantity = yew_amt;
							end
						elseif featureType == FeatureTypes.NO_FEATURE then
							if plotType == PlotTypes.PLOT_HILLS then
							...
							elseif terrainType == TerrainTypes.TERRAIN_DESERT then
								...
								local diceroll = Map.Rand(2, "Resource selection - Place Small Quantities LUA");
								if diceroll == 0 then
									selected_ID = self.iron_ID;
									selected_quantity = iron_amt;
								else
...
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceFish(frequency, plot_list)	--Paz: added crabs and latitude check
			...
			for index_to_check = current_index, iNumTotalPlots do
				...
				if self.fishData[plotIndex] == 0 then
					...
					if res_plot:GetResourceType(-1) == -1 then
						...
						--Paz: use Crab if coastal AND extreem latitude
						local latitude = (y - (iH / 2)) / (iH / 2)
						local terrainType = res_plot:GetTerrainType()
						if terrainType == TerrainTypes.TERRAIN_COAST and (latitude > 0.65 or latitude < -0.65) then 
							res_plot:SetResourceType(self.crab_ID, 1);
...
function AssignStartingPlots:PlaceSexyBonusAtCivStarts()
	...
	local bonus_type_associated_with_region_type = {self.deer_ID, self.boars_ID, 
	self.deer_ID, self.wheat_ID, self.berries_ID, self.wheat_ID, self.cow_ID, self.cow_ID};			--Paz swapped berries for sheep
	...
	for region_number = 1, self.iNumCivs do
			...
			for plot_to_handle = 1, ripple_radius do
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS then -- No resource or Oasis here, safe to proceed.
						...
						elseif use_this_ID == self.boars_ID then
							if featureType == FeatureTypes.FEATURE_JUNGLE then
								table.insert(plot_list, plotIndex);
							end
						...
						elseif use_this_ID == self.berries_ID then
		...
		if iNumCandidates > 0 then
			...
			local iNumLeftToPlace = self:PlaceSpecificNumberOfResources(use_this_ID, 1, 1, 1, -1, 0, 0, shuf_list);
			if iNumCandidates > 1 and use_this_ID == self.berries_ID then
...
function AssignStartingPlots:AddModernMinorStrategicsToCityStates()
	...
	for city_state = 1, self.iNumCityStates do
		...
		if self.city_state_validity_table[city_state] == false then
			...
			local x = self.cityStatePlots[city_state][1];
			local y = self.cityStatePlots[city_state][2];
			-- Choose strategic type. Options are: 0 None, 1 Copper, 2 Naphtha, 3 Mithril
...
------------------------------------------------------------------------------
--[[
function AssignStartingPlots:PlaceNaphthaInTheSea()
	-- Places sources of Naphtha in Coastal waters, equal to half what's on the 
	...
	local sea_naphtha_amt = 4;
	if self.resource_setting == 3 then -- Abundant Resources, increase amount.
		sea_naphtha_amt = 6;
	end
	local iNumLandNaphthaUnits = self.amounts_of_resources_placed[self.naphtha_ID + 1];
	local iNumToPlace = math.floor((iNumLandNaphthaUnits / 2) / sea_naphtha_amt);

	--print("Adding Naphtha resources to the Sea.");
	self:PlaceSpecificNumberOfResources(self.naphtha_ID, sea_naphtha_amt, iNumToPlace, 0.2, 1, 4, 7, self.coast_list)
end
]]
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--Paz add
function AssignStartingPlots:RemoveBerriesInDesert()
	-- Yes, this is stupid. But I'm tired of fighting with it...
	-- Deserts should suck anyway.
	local iW, iH = Map.GetGridSize()
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			local plot = Map.GetPlot(x, y)
			if plot:GetResourceType(-1) == self.berries_ID then
				local terrainType = plot:GetTerrainType();
				if terrainType == TerrainTypes.TERRAIN_DESERT then
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	--print("- Paz LUXURY Resources -");
	print("- Tobacco.: ", self.amounts_of_resources_placed[self.tobacco_ID + 1]);
	print("- Tea.....: ", self.amounts_of_resources_placed[self.tea_ID + 1]);
	print("- Opium...: ", self.amounts_of_resources_placed[self.opium_ID + 1]);
	print("- Sheep...: ", self.amounts_of_resources_placed[self.sheep_ID + 1]);
	print("- LUXURY-like Resources -");
	print("- Elephants: ", self.amounts_of_resources_placed[self.elephants_ID + 1]);
	...
	print("- Mithril.: ", self.amounts_of_resources_placed[self.mithril_ID + 1]);
	print("- Naphtha: ", self.amounts_of_resources_placed[self.naphtha_ID + 1]);
	print("- Yew.....: ", self.amounts_of_resources_placed[self.yew_ID + 1]);
	print("- STRATEGIC Magic Resources -");
	print("- Cinnabar.: ", self.amounts_of_resources_placed[self.cinnabar_ID + 1]);
	print("- Moly.....: ", self.amounts_of_resources_placed[self.moly_ID + 1]);
	print("-");
	...
	print("- Boars: ", self.amounts_of_resources_placed[self.boars_ID + 1]);
...
function AssignStartingPlots:GetMajorStrategicResourceQuantityValues()
	...
	-- Note: scripts that cannot place Naphtha in the sea need to increase amounts on land to compensate.
	local cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 3, 6, 3, 6, 8, 2, 4;
	-- Check the resource setting.
	if self.resource_setting == 1 then -- Sparse
		cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 2, 4, 2, 4, 6, 1, 2;
	elseif self.resource_setting == 3 then -- Abundant
		cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 4, 8, 4, 8, 10, 3, 6;
	end
	return cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt
end
------------------------------------------------------------------------------
function AssignStartingPlots:GetSmallStrategicResourceQuantityValues()
	...
	local cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 2, 4, 2, 4, 6, 1, 2;
	-- Check the resource setting.
	if self.resource_setting == 1 then -- Sparse
		cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 1, 2, 1, 2, 3, 1, 1;
	elseif self.resource_setting == 3 then -- Abundant
		cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt = 3, 6, 3, 6, 8, 2, 3;
	end
	return cinnabar_amt, horse_amt, naphtha_amt, iron_amt, copper_amt, mithril_amt, yew_amt
end
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	{self.copper_ID, copper_amt, 35, 1, 3},
	{self.mithril_ID, mithril_amt, 39, 2, 3} };
	self:ProcessResourceList(22, 1, self.hills_list, resources_to_place)

	local resources_to_place = {
	{self.copper_ID, copper_amt, 100, 1, 2} };
	self:ProcessResourceList(33, 1, self.jungle_flat_list, resources_to_place)
	local resources_to_place = {
	{self.copper_ID, copper_amt, 35, 1, 2},
	{self.yew_ID, yew_amt, 65, 1, 3} };	--Paz add
	self:ProcessResourceList(39, 1, self.forest_flat_list, resources_to_place)
	local resources_to_place = {
	{self.yew_ID, yew_amt, 100, 1, 3} };
	self:ProcessResourceList(39, 1, self.hills_forest_list, resources_to_place)	--Paz add
	...
	--Paz diable:	self:PlaceNaphthaInTheSea();

	
	-- Check for low or missing Strategic resources
	--Paz altered minimums
	...
	if self.amounts_of_resources_placed[self.naphtha_ID + 1] < 4 * self.iNumCivs then
		--print("Map has very low naphtha, adding another.");
		local resources_to_place = { {self.naphtha_ID, naphtha_amt, 100, 0, 0} };
		self:ProcessResourceList(99999, 1, self.land_list, resources_to_place)
	end
	if self.amounts_of_resources_placed[self.mithril_ID + 1] < 4 * self.iNumCivs then
		--print("Map has very low mithril, adding another.");
		local resources_to_place = { {self.mithril_ID, mithril_amt, 100, 0, 0} };
		self:ProcessResourceList(99999, 1, self.hills_list, resources_to_place)
	end
	if self.amounts_of_resources_placed[self.yew_ID + 1] < 4 * self.iNumCivs then	--Paz added yew
		--print("Map has very low cinnabar, adding another.");
		local resources_to_place = { {self.yew_ID, yew_amt, 100, 0, 0} };
	...
	local resources_to_place = {			--Paz: yes, boars in the banana list!
	{self.boars_ID, 1, 100, 0, 3} };
	self:ProcessResourceList(14 * bonus_multiplier, 3, self.banana_list, resources_to_place)
...
-j. Various cleanup operations occur, including placing Naphtha in the sea, 
...
This method also handles placement of sea-based sources of Naphtha.
